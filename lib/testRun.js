...
module.exports = function() {
  ...
  function parseAnnotations(testName) {
    ...
    do {
      ...
      if (text.match(/^[a-z0-9]+$/)) {
        ...
        var active = parseAnnotationValue(text.toLowerCase(), /active\=[\"\']?([1|0|true|false|yes|no])[\"\']?/);
        if (active) {
          annotation.active = active === 'true' || active === '1' || active === 'yes';
        }
  ...
  _.extend(TestRun.prototype, {
    ...
    add: function(key, name, passed, duration, options) {
      ...
      if (!_.isUndefined(annotation.active)) {
        result.active = annotation.active;
      }
